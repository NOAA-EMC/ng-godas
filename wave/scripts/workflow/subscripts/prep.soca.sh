...
sed -i "s;__DA_VARIABLES_WAV__;$DA_VARIABLES_WAV;g" soca_staticbinit.yaml
...
ln -s $SOCA_BIN_DIR/soca_{gridgen,error_covariance_training,staticbinit}.x .
...
[[ "$DA_ENABLED" == T ]] && {
    ...
    if [[ -d $SOCA_STATIC_DIR/bump || -d $SOCA_STATIC_DIR/bump_sub ]]; then
        ...
        [[ "$DA_DUALRES_ENABLE" == "F" ]] && {
           ...
           if [[ "$DA_MODE" == "3dhyb" ]]; then
               $MPIRUN ./soca_error_covariance_training.x soca_locinit.yaml
           fi
