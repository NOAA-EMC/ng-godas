...
case "$DA_MODE" in
  ...
  3dhyb)
    ...
    if [[ "$DA_ENSBDIAG_ENABLED" =~ [yYtT1] ]]; then
        ...
        ln -s $DIAGB_TMP_DIR/wav.ens_variance.*.nc wav.bkgerror.nc
    else
...
sed -i "s;__DA_VARIABLES_WAV__;$DA_VARIABLES_WAV;g" var.yaml
...
if [[ "$DA_WAV_ENABLED" == [yYtT1] ]]; then
   ln -sf Data/wav.3dvar.an*.nc wav.checkpoint_ana.nc
fi
...
# Dump wav analysis into ww3 restart
if [[ "$DA_WAV_ENABLED" =~ [yYtT1] ]]; then
   cp $SOCA_SCIENCE_DIR/tools/wav/* ./
   ln -fs ./mod_def.$WAV_MODEL_RES ./mod_def.ww3
   ln -fs $BKGRST_DIR/*restart.$WAV_MODEL_RES ./restart.ww3
   ...
   #
   sed -i "s+SYEAR+${SYEAR}+g" ww3_uprstr.inp
   sed -i "s+SMONTH+${SMONTH}+g" ww3_uprstr.inp
   sed -i "s+SDAY+${SDAY}+g" ww3_uprstr.inp
   sed -i "s+SHOUR+${SHOUR}+g" ww3_uprstr.inp
   #
   python soca_gen_anl_grbtxt.py -i wav.checkpoint_ana.nc 
   $WW3_BIN_DIR/ww3_uprstr ww3_uprstr.inp
   cp restart001.ww3 $ANARST_DIR/
   cp restart001.ww3 $BKGRST_DIR/
   cp wav.checkpoint_ana.nc $BKGRST_DIR/
   #cp wav.checkpoint_ana.nc $ANARST_DIR/ 
