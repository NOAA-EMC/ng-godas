...
[[ "$DA_WAV_ENABLED" =~ [tTyT1] ]] && ncks -O -7 -L 4 \
    --ppc default=.2 -v $WAV_STATE_VARS \
    $BKGRST_DIR/wav.res.nc \
    $BKG_DIR/ctrl/wav.bkg.$ymdh.nc
...
if [[ "$DA_ENABLED" == "T" ]]; then
    ...
    [[ "$DA_WAV_ENABLED" =~ [tTyT1] ]] && ncks -O -7 -L 4 \
        --ppc default=.2 -v $WAV_STATE_VARS \
        $ANARST_DIR/wav.res.nc \
        $ANA_DIR/ctrl/wav.ana.$ymdh.nc
...
if [[ "$DA_LETKF_ENABLED" =~ [yYtT1] ]]; then
    ...
    [[ "$DA_WAV_ENABLED" =~ [tTyT1] ]] &&
        ncks -O -7 -L 4 --ppc default=.2 -v $WAV_STATE_VARS \
             $ANA_ENS_DIR/wav.letkf.ens.0.nc \
             $ANA_DIR/ens/wav.ana.ens_mean.$ymdh.nc
    ...
    if [[ "$DA_WAV_ENABLED" =~ [tTyT1] ]]; then
        for ens in $(seq $DA_ENS_SIZE); do
            ncbo -O $ANA_ENS_DIR/wav.letkf.ens.$ens.nc \
                $ANA_ENS_DIR/wav.letkf.ens.0.nc \
                ana_sprd/wav.$ens.nc
        done
        nces -y rmssdn -O -7 -L 4 --ppc default=.4 \
            ana_sprd/wav.*.nc \
            $ANA_DIR/ens/wav.ana.ens_sprd.$ymdh.nc
    fi
    ...
    [[ "$DA_WAV_ENABLED" =~ [tTyT1] ]] &&
       nces  -O -7 -L 4 --ppc default=.2 -v $WAV_STATE_VARS \
             $BKGRST_ENS_DIR/???/wav.res.nc \
             $BKG_DIR/ens/wav.bkg.ens_mean.$ymdh.nc
    ...
    if [[ "$DA_WAV_ENABLED" =~ [tTyT1] ]]; then
        for ens in $(seq -f "%03g" $DA_ENS_SIZE); do
            ncbo -O -v $WAV_STATE_VARS $BKGRST_ENS_DIR/$ens/wav.res.nc \
                                   $BKG_DIR/ens/wav.bkg.ens_mean.$ymdh.nc \
                                   bkg_sprd/$ens.nc
        done
        nces -y rmssdn -O -7 -L 4 --ppc default=.4 bkg_sprd/*.nc $BKG_DIR/ens/wav.bkg.ens_sprd.$ymdh.nc
    fi
...
if [[ "$SAVE_DIAG_ENABLED" =~ [yYtT1] ]]; then
    ...
    for DIAG_FILE in "$DIAG_TMP_DIR"/*ocean_diag*
    do
