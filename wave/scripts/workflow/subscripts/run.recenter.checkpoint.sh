...
if [[ "$DA_MODE" == letkf ]]; then
    ...
    for ens in $(seq -f "%03g" $DA_ENS_SIZE ); do
        ...
        if [[ "$DA_WAV_ENABLED" =~ [yYtT1] ]]; then
           echo "    wav_filename: ./$ens/wav.res.nc" >> ens.tmp
        fi
    done
...
for ens in $(seq $mem0 $DA_ENS_SIZE); do
    ...
    if [[ "$DA_WAV_ENABLED" =~ [yYtT1] ]]; then
       ln -sf $WORK_DIR/ana_ens/wav.${output_descriptor}.ens.$((10#$ens)).*nc wav.checkpoint_ana.nc
    fi
    ...
    if [[ "$DA_WAV_ENABLED" =~ [yYtT1] ]]; then
       ...
       ln -fs $bkg_dir/*restart.$WAV_MODEL_RES ./restart.ww3
       ...
       cp restart001.ww3 $ana_dir/
       cp restart001.ww3 $bkg_dir/
       cp wav.checkpoint_ana.nc $ana_dir/
       cp wav.checkpoint_ana.nc $bkg_dir/
