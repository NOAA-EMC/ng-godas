...
int main(int argc, char *argv[]) {
  ...
  for (auto var_pair : vars) {
    ...
    if (root) {
      int r_size_total = 0;
      for (int i =0; i < mpi_size; i++) {
        r_size[i] = nlocs == 0 ? 0 : p_nlocs[i] * s_size/nlocs;
  ...
  if ( root ) {
    ...
    for (auto attr : ncin.getAtts()) {
      void * bufr = malloc(attr.second.getAttLength());
      attr.second.getValues(bufr);
      ncout->putAtt(attr.first, attr.second.getType(),
                    attr.second.getAttLength(), bufr);
      free(bufr);
    }
    ...
    for (auto grp : ncin.getGroups()) {
      ...
      auto out_grp = ncout->addGroup(grp.first);

      // copy over the attributes
      for (auto attr : grp.second.getAtts()) {
        void * bufr = malloc(attr.second.getAttLength());
        attr.second.getValues(bufr);
        out_grp.putAtt(attr.first, attr.second.getType(),
                       attr.second.getAttLength(), bufr);
        free(bufr);
      }
    }
    ...
    for (auto var : ncin.getVars(netCDF::NcGroup::Location::All)) {
      ...
      // copy over the attributes
      for (auto attr : var.second.getAtts()) {
        void * bufr = malloc(attr.second.getAttLength());
        attr.second.getValues(bufr);
        v.putAtt(attr.first, attr.second.getType(),
                 attr.second.getAttLength(), bufr);
        free(bufr);
      }
    }
